...
describe("Persistent Node Chat Server", function() {
  ...
  it("Should insert posted messages to the DB", function(done) {
    ...
    request({method: "POST",
            ...
            function(error, response, body) {
              ...
              // console.log("AAAAAAA", error, response, body);

              var queryString = "SELECT users.name, messages.content FROM messages INNER JOIN users ON users.ID=messages.userID";
              var queryArgs = [];
  ...
  it("Should output all messages from the DB", function(done) {
    ...
    var queryString = "INSERT INTO messages (userID, content, roomID) VALUES (?, ?, ?)";
    var queryArgs = [3, "Men like you can never change!", 1];
      ...
      function(err, results, fields) {
        ...
        request("http://127.0.0.1:3000/classes/messages",
          function(error, response, body) {
            ...
            expect(messageLog[0].userID).to.equal(3);
